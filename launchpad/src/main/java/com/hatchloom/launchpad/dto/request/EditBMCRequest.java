 ...
 *
 * <p>{@code section} must be one of the 9 valid BMC section keys
 * (e.g. {@code "value_propositions"}, {@code "key_partners"}).
 * Unknown keys are rejected with 400 Bad Request.</p>
 */
public class EditBMCRequest {

    @NotBlank
    @Pattern(
        regexp = "(?i)key_partners|key_activities|key_resources|value_propositions"
                + "|customer_relationships|channels|customer_segments|cost_structure|revenue_streams",
        message = "section must be one of: key_partners, key_activities, key_resources, "
